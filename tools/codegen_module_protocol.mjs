...
for (const module of protocolModules) {
  ...
  await runNode(godotArguments, root);
  ...
  process.stdout.write(
    `[codegen:module-protocol] ${checkOnly ? "checked" : "generated"} ${module.id} messages=${schemaLock.entries.length} ` +
      `typescript=${relative(protocol.typescriptOutput)} godot=${relative(path.join(protocol.godotOutput, `${protocol.godotClassName}.gd`))}\n`,
  );
}
...
async function validateGlobalOpcodeOwnership(records) {
  ...
  const hostLock = await readLock(hostLockFile, "host opcode");
...
async function writeProtocolManifest({
  ...
  await writeFile(
    path.join(module.root, "protocol.manifest.json"),
