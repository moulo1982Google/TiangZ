...
/**
 * 对开发人员隐藏重复构建步骤，并把 Hotfix 源码变化串行发布给 Watcher。
 * Hides repetitive build steps from developers and serially publishes Hotfix source changes to the Watcher.
 *
 * 副作用：启动子进程、监听 app/hotfix，并在退出时请求 Watcher 优雅停机；不得用于正式部署。
 * Side effects: starts child processes, watches app/hotfix, and requests graceful Watcher shutdown on exit; do not use for production deployment.
 */
async function main() {
  ...
  const sourceWatcher = watch(hotfixRoot, { recursive: true }, (_event, filename) => {
    ...
    clearTimeout(timer);
    timer = setTimeout(() => void rebuildAndReload(), debounceMs);
  });
  ...
  process.stdout.write("[dev] 正在监听 app/hotfix/**/*.ts；保存后将自动构建并 Reload。\n");
  ...
  async function rebuildAndReload() {
    ...
    try {
      do {
        ...
        runtime.stdin.write(`reload ${absoluteCandidate}\n`);
        process.stdout.write(`[dev] 已提交 Reload：${candidate}\n`);
      } while (pending && !stopping);
    } catch (error) {
      process.stderr.write(`[dev] Hotfix 未发布，旧 generation 继续运行：${errorMessage(error)}\n`);
    } finally {
      building = false;
    }
  }
