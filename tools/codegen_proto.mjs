...
function parseProto(source, fileInfo, opcodeLock) {
  ...
  for (const messageMatch of source.matchAll(messagePattern)) {
    ...
    // Fields may share a line. Remove comments without consuming quoted option
    // values so a commented-out declaration never enters the generated schema.
    const fieldBody = body.replace(/"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|\/\*[\s\S]*?\*\/|\/\/[^\n]*/g,
      token => token.startsWith("/") ? token.replace(/[^\n]/g, " ") : token);
    for (const fieldMatch of fieldBody.matchAll(fieldPattern)) {
