...
const REQUEST_MSGCODE: u16 = 15_004;
const RESPONSE_MSGCODE: u16 = 15_005;
...
async fn main() -> Result<()> {
    ...
    for index in 0..options.connections {
        ...
        tasks.push(tokio::spawn(async move {
            run_connection(options, window, barrier, timing).await
        }));
    }
    ...
    let results = tokio::time::timeout(overall_timeout, async {
        ...
        for task in tasks {
            results.push(task.await.context("inner RPC load task panicked")??);
        }
        Result::<Vec<ConnectionResult>>::Ok(results)
    })
    .await
    .context("inner RPC load test timed out")??;

    let max_server_concurrency = results
        .iter()
        .map(|item| item.max_server_concurrency)
        .max()
        .unwrap_or_default();
    ...
    println!("Internal scene RPC load test (Rust client)");
    println!(
        "target={}:{} duration={}s warmup={}s connections={} concurrency={} call_count={} delay={}ms",
        options.host,
        options.port,
    ...
    println!(
        "requests={requests} req/s={requests_per_second:.0} errors=0 peak_in_flight={} max_server_concurrency={max_server_concurrency}",
        options.concurrency,
    );
    println!(
...
async fn run_connection(
    ...
    while !pending.is_empty() {
        ...
        result.max_server_concurrency = result
            .max_server_concurrency
            .max(response.max_server_concurrency);
        if request.measured {
            result
                .latencies_micros
...
async fn send_request(
    ...
    *next_rpc_id = next_rpc_id.wrapping_add(1).max(1);
    ...
    let packet = encode_request(rpc_id, options.call_count, options.delay_ms)?;
...
fn encode_request(rpc_id: u32, call_count: u32, delay_ms: u32) -> Result<Vec<u8>> {
    ...
    push_field_varint(&mut frame, 1, call_count);
    push_field_varint(&mut frame, 2, delay_ms);
...
fn decode_response(frame: &[u8]) -> Result<DecodedResponse> {
    ...
    while offset < frame.len() {
        ...
        match (field_number, wire_type) {
            ...
            (2, 0) => response.max_server_concurrency = read_varint(frame, &mut offset)? as u32,
            (_, 0) => {
...
fn parse_options(args: Vec<String>) -> Result<Options> {
    ...
    Ok(Options {
        ...
        port: u16::try_from(number("port", 7410)?).context("port exceeds uint16")?,
        duration: Duration::from_secs(duration),
        warmup: Duration::from_secs(warmup),
        concurrency,
        connections,
        call_count: u32::try_from(number("call-count", 1)?).context("call-count exceeds uint32")?,
