...
当前 Windows 开发机的完整业务容量基线为：单 MapHost、10 Gate、3500 名玩家持续执行 `2Hz` 移动、`0.2Hz` Probe 和 `0.1Hz` 道具/技能业务时，Map 保持 `20Hz` 且正式窗口零跳帧，Probe P95/P99 为 `183/220ms`，窗口内无传输错误、过载、超时或背压。该结果是玩家均匀分布在 `10x10` AOI Grid、特定测试机和拓扑下的回归基线，不是生产容量承诺。
