...
持久化边界位于独立的 [TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy) Rust 仓库。当前`v0.3.1`已经通过Rust Host连接池接入首个Player Snapshot Repository，并完成TiangZ重启后的PostgreSQL恢复验收；主工程只依赖协议、客户端SDK和业务Repository，不直接连接Redis/PostgreSQL。关键经济事务、周期快照、批量恢复和生产高可用仍属于Phase 4.5后续工作，运行步骤见[DBProxy玩家快照持久化](docs/tutorials/19-dbproxy-player-persistence.md)。
