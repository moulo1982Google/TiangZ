...
Phase 4.2.3已经把官方Recast/Detour `v1.6.0`资产、Rust权威移动和AOI下行接入Map Runtime：Map 100启动时校验SHA-256并共享只读NavMesh，每个MapInstance独占查询、路径和AOI状态；TS通过粗粒度`ProjectPosition/FindPath/NavigateTo/NavigateInput`调用，不接触Detour句柄。`cocos_client3D`可登录Map 100，支持左键寻路、W/S前后、A/D转向、按住右键时A/D横移和尾随相机，并以同一权威Push完成本地纠偏与AOI远端插值。射线、独立高度查询和动态障碍继续按Phase 4.2推进。详见[NavMesh3D运行时与Cocos灰盒](docs/tutorials/13-navmesh3d.md)。
