...
- `gateName/gateEpoch`：当前Gate所有者与单调递增代次；普通同Gate重连不改变，故障接管递增。
...
5. Gate故障接管由权威PlayerUnit邮箱调用`RebindGate`，同时校验revision、Actor、旧Gate、旧epoch、MapHost所有权代次和operationId；只切Gate，不迁移Actor或地图。

玩家Location目录之外还有独立的MapInstance目录，只保存`mapInstanceId -> mapConfigId + mapHostName + dynamic + ownerGeneration`。静态地图令`mapInstanceId == mapConfigId`且路由不超时；动态副本使用`GlobalIdSystem`产生全局实例号和15秒租约。MapHost每5秒重报实际托管实例，因此Location重启后可恢复路由。业务传送只提供目标实例号，不能扫描全部MapHost，也不能把MapHost地址塞入玩家传送请求。
...
- 另一个Gate接管：旧Gate探测失败后按Location快照发起一次CAS；成功后后续普通消息仍不查询Location。
...
Gate故障接管已经在同一已知拓扑内实现：Login保留当前健康Gate，失败时选择另一健康Gate；Location以CAS递增gateEpoch；ActorLocation固定头携带epoch并在PlayerUnit邮箱入口拒绝旧Gate消息；Native AOI只换delivery route，不重建可见关系。接管不迁移Socket，也不自动回切。

ActorLocation、ActorLocationBatch和Trace是Core内部二进制信封，不属于业务protobuf opcode。它们的msgcode和字段offset集中定义在`InternalFrameProtocol.ts`，协议生成器把全部内部msgcode视为保留值并纳入生成指纹；encode/decode不得各自手写裸offset。修改内部布局至少要更新golden vector、截断边界和随机round-trip测试，并验证Rust消费者兼容性；格式数量或跨语言消费者明显增加前，不为统一外观强行改成业务protobuf。

动态副本采用安全回退而不是现场恢复：Manager或Location单独重启可由存活MapHost重报；Manager与MapHost双失后，Location租约过期并删除旧玩家Actor路由，Gate重连后进入初始静态地图。怪物、Boss、仇恨、AI和战斗计时器直接丢弃，已确认的背包、货币、任务和成长从DBProxy恢复。

当前尚未实现：跨机器租约仲裁、动态副本现场恢复、在途跨进程事务日志和etcd服务发现。因此现有Gate、静态MapHost接管和动态副本安全回退仍是单集群显式策略，不等于跨地域生产级分布式高可用。
...
- `npm run test:gate-failover`：双Gate真实强杀、同Unit接管、旧Gate有界重启、禁止自动回切与旧Gate重入拒绝。
- `npm run test:dynamic-map-fallback`：同时强杀Manager和动态MapHost，等待租约过期后重连到安全静态地图，并确认Watcher恢复两个进程。
