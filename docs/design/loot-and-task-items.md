...
掉落属于尸体和地图，不属于玩家，也不是`MonsterUnit`上的可传送子Entity。尸体销毁时，容器一起清理。玩家不能因为看到尸体快照就本地增加Item或Quest进度。
...
- 死亡时刷怪槽立即释放，并在达到`respawn_seconds`后生成新的`MonsterUnit`；旧尸体继续用旧`UnitId`留在独立尸体集合和AOI中，两者可以同时存在。
- 普通掉落全部领取后，旧尸体可以立即离开AOI；这不会提前或延后同槽新怪的刷新。
...
运行时内容包还可以为普通行声明正数`groupId`，同组候选最多选中一行。组内先用一次抽样按顺序选择正概率成员；全部未命中后，`chancePermille=0`的候选再用一次独立抽样等概率兜底。组内所有行可以再共享一个`groupGatePermille`，表示进入组内选择前的千分比门槛。`groupGatePermille=0`或省略表示不设门槛，等价于1000；同一组声明不同门槛、独立行声明非零门槛、显式概率总和超过1000、或等权成员没有剩余概率都会在内容登记或运行时判定时被拒绝。组门槛、显式选择和等权兜底使用彼此独立的确定性抽样通道，不能复用一次随机数，否则会使部分兜底成员偏斜甚至永远无法命中。

普通行还可以用`nestedDropTableId`表示“本行命中后执行一次子掉落表”。这类包装行不能同时发放物品/金币、不能带任务资格，执行次数固定为一次；被引用表必须存在、不能包含任务资格行，整个有向内容图最多16层且不能成环。每一层仍复用相同的独立行、互斥组和组门槛规则，最后只把叶子奖励写入尸体或交互事务。该契约用于通用的分层奖励表，不携带任何来源数据库的引用编号或游戏专有语义。
