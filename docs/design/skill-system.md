...
第一版演示七个法术，全部触发1秒公共冷却，并在技能被接受时清零平A进度；读条期间暂停平A，成功或中断后都从0重新计时，但不关闭平A激活状态或清除目标。
...
7. `3007 精神鞭笞`：敌方Unit目标、30米、5秒引导、每1秒造成20点暗影伤害、共5跳；移动会取消引导，受到一次有效攻击时将结束时间提前800毫秒，提前到当前服务器时间时立即结束。

### 11.1 精神鞭笞的引导语义

`3007`不是“每秒创建一个Timer”，而是一个由`SkillMapComponent.Update10Hz()`推进的普通`ActiveCast`：

- 开始时冻结5秒结束时间和5次Tick规则，成功施放立即提交1秒GCD，不额外设置技能CD。
- 每到一个服务器Tick，先按当前目标ID重新取目标，再执行20点暗影伤害；已完成的Tick不会因为受击而重放或回滚。
- 玩家发生有效移动输入时直接中断，目标受到一次有效攻击时只把`finishAtMs`提前800ms，不清除当前Cast、不重置起点，也不改变已完成Tick数量。
- 如果提前后的结束时间早于下一跳，剩余Tick不强行补发；这是“缩短引导时间”，不是“保证五次伤害”。
- 当前Demo的有效攻击来源是怪物普通攻击；以后玩家技能或其他战斗来源接入时，必须继续调用同一个`HandleDamageDuringCast`边界，不能复制一套缩短逻辑。
