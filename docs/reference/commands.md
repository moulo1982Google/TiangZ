...
独立游戏可直接运行 `TiangZ configs/dev/all-in-one.json --runtime-root=release-game`，其中 `release-game` 是相对于当前目录的游戏资源目录，也可提供绝对路径（含空格时给整个参数加引号）。该目录必须包含 `dist/` 和 `configs/`；显式路径无效直接退出，不回退主工程。配置相对该目录解析，Watcher 子进程继承它。省略选项保留原启动目录推断；不改变 Bundle/模块兼容检查。重复根目录选项、多个配置位置及未知参数会报错。
...
| `npm run asset:cocos3d:blue-chibi` | 使用Blender 5.2 LTS重新生成Cocos3D蓝发角色GLB和可编辑Blend源文件 |
