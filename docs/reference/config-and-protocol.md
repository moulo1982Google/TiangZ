...
| `environment` | string? | 部署环境：`development`、`test`、`staging`、`production`，缺省 `development`，其他取值拒绝启动。宿主投影给 TS 的 `ProcessRuntimeInfo.Instance.Environment`，并在健康端口 `/runtime-identity` 返回；框架不解释它，按环境切换的行为由业务决定。生产配置应显式填写，部署工具应核对它与部署规格一致（0.6.2） |
