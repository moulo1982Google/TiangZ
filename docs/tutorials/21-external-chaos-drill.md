...
| 游戏负载 | 500 玩家，两个 Node 子进程各 250；每 5 分钟重建一次全链路连接；1 Hz 移动、0.05 Hz 探测、0.02 Hz 业务 |
...
长稳 runner 每 5 分钟完成一个 epoch，并用稳定账号重新走完整连接链路。地图 `1` 的进图响应声明 `Grid2D`，客户端据此使用 `C2M_Move/G2C_EntityMove`；地图 `100` 声明 `NavMesh3D`，客户端自动改用 `C2M_NavigateInput/M2C_NavigateInput`，并验证 `G2C_EntityNavigate` 权威 Push。不能只按进程退出码判断通过，否则错误地向 NavMesh3D 地图发送 Grid2D 消息也可能得到退出码 `0`。
...
这些结果只说明编译、协议选择、基础持久化链路、单 peer 切换和安全开关已经就绪，不能替代 100 玩家完整故障序列、500 玩家 2–4 小时和最终 168 小时验收。
