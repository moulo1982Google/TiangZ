...
| `respawn_seconds` | 从死亡时刻计时的新怪重生秒数；不等待旧尸体窗口结束 |

`MonsterAreaConfig`描述“在哪里刷”：一行就是一个固定刷怪槽位，包含地图配置ID、模板ID、坐标和是否在地图创建时生成。复活时间属于怪物模板，不属于刷点，避免同一个怪物模板在不同地图拥有隐含的生命周期差异。刷怪槽只引用当前活怪；死亡Unit会进入独立尸体集合，所以同一刷点可以同时存在一只新活怪和一具仍可拾取的旧尸体。当前版本没有随机刷怪池和多个候选点。

3D演示地图`MapConfig.id = 100`配置三个怪A（被动）和两个怪B（主动）刷点。各位置避开NPC出生区、中央障碍和动态门，Cocos3D进入`Map 100`后可以观察两种颜色。
