...
同日后续：DBProxy `SaveMulti` 的连接分片都指向同一 PG，现将批次按首记录路由到一条连接、一次提交，批内按记录排序、回执恢复原顺序；逐条 CAS/幂等与 CommitRecords 边界不变。清理修复首轮已通过启动但在第 18 分钟健康检查失败，须以新候选重跑完整验收。
...
2026-09-07 换机续接：缓存预算优化已验证，PG 故障排队优化尚未实施；先读[交接入口](../testing/handoff-2026-09-07.md)，同步相邻 DBProxy 仓库，不自动重启长稳。
