...
- Starter动态副本固定使用MapConfig 200。客户端只向Gate提交稳定`operationId`；Gate通过`DynamicMapProxy`以`starter-dungeon:<characterId>:<operationId>`幂等请求MapManager分配实例，再复用普通`EnterMapCore`进入。副本内MonsterConfig 3“试炼守卫”拥有900生命，不掉落物品；死亡后发布`MonsterEvents.Killed`，副本领域监听器给击杀玩家增加120累计经验。经验属于`NumericType.Experience`，等级阈值为`50 * (level - 1) * level`、上限60；奖励先通过DBProxy只提交`progression`领域，再修改在线Numeric并推送`G2C_ProgressionChanged`。空副本继续使用现有5分钟无人回收兜底，不恢复崩溃前的Boss现场。
