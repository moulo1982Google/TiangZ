...
- Phase 4.5最后建设持久化基础，再进入正式账号、角色和经济业务。独立仓库[TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy)当前为`v0.1.5`，提供`dbproxy-core`的`RecordKey`、快照Payload、Revision/CAS、幂等请求、单记录`TransactionalWrite`和进程内`SnapshotFlushQueue`，以及`dbproxy-storage`的PostgreSQL权威存储、Redis已提交快照缓存、缓存回源、缓存修复和PostgreSQL故障后的快照重试；本机Docker Compose集成测试与Redis/PostgreSQL短暂停机矩阵已通过。普通快照队列只在DBProxy进程内存中存在，支持按记录合并、限制每轮写入量和有限轮停机排空，不能宣称覆盖DBProxy自身重启。它不认识Scene、Entity、Component、Buff、Hotfix或`.native`。网络服务、鉴权、Rust/TypeScript SDK、Redis持久积压队列和生产部署仍未实现，主工程不得直接连接Redis/DB。TiangZ未来只生成领域Payload、快照codec和Repository适配层，不导入DBProxy内部模块。下一步再完成Redis-backed durable backlog、DBProxy重启恢复、网络协议和生成Repository。`.native`仍在TiangZ按Entity/Component声明`transient/snapshot/transactional`并生成dirty、schema和恢复入口；同一字段不得拥有两条权威写路径。
