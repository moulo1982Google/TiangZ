...
- Phase 4.5最后建设持久化基础，再进入正式账号、角色和经济业务。独立仓库[TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy)当前为`v0.1.4`，提供`dbproxy-core`的`RecordKey`、快照Payload、Revision/CAS、幂等请求和单记录`TransactionalWrite`，以及`dbproxy-storage`的PostgreSQL权威存储、Redis已提交快照缓存、缓存回源和故障修复入口；本机Docker Compose集成测试与Redis/PostgreSQL短暂停机矩阵已通过。它不认识Scene、Entity、Component、Buff、Hotfix或`.native`。网络服务、鉴权、Rust/TypeScript SDK、积压/Flush恢复和生产部署仍未实现，主工程不得直接连接Redis/DB。TiangZ未来只生成领域Payload、快照codec和Repository适配层，不导入DBProxy内部模块。后续先完成积压/下线Flush故障矩阵，再实现网络协议和生成Repository。`.native`仍在TiangZ按Entity/Component声明`transient/snapshot/transactional`并生成dirty、schema和恢复入口；同一字段不得拥有两条权威写路径。
