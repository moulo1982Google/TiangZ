...
- Phase 4.5最后建设持久化基础，再进入正式账号、角色和经济业务。独立仓库[TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy)当前为`v0.1.6`，提供`dbproxy-core`的`RecordKey`、快照Payload、Revision/CAS、幂等请求、单记录`TransactionalWrite`和进程内`SnapshotFlushQueue`，以及`dbproxy-storage`的PostgreSQL权威存储、Redis已提交快照缓存、缓存回源、缓存修复、PostgreSQL故障后的快照重试和Redis AOF持久`SnapshotBacklog`；本机Docker Compose集成测试、Redis重启恢复和Redis/PostgreSQL短暂停机矩阵已通过。普通内存队列只在DBProxy进程内存中存在；Redis backlog通过lease、ACK和过期回收支持DBProxy重启后重新领取，但仍依赖Redis AOF/数据卷，不等于Redis高可用。它不认识Scene、Entity、Component、Buff、Hotfix或`.native`。网络服务、鉴权、Rust/TypeScript SDK、长时间故障监控和生产部署仍未实现，主工程不得直接连接Redis/DB。TiangZ未来只生成领域Payload、快照codec和Repository适配层，不导入DBProxy内部模块。下一步完成网络协议、backlog指标和生成Repository。`.native`仍在TiangZ按Entity/Component声明`transient/snapshot/transactional`并生成dirty、schema和恢复入口；同一字段不得拥有两条权威写路径。
