...
- 当前业务缺口按Boss副本奖励、透明节点接管和正式Hotfix入口排序；无DBProxy时角色目录只保证进程生命周期内一致，重启恢复必须使用DBProxy。DBProxy v0.5.0的多Endpoint、双实例和TiangZ端到端故障切换已验收；当前MapHost崩溃语义是Watcher收束整组部署后安全重启恢复，不是透明热接管。
...
- Phase 4.5已完成玩家持久化第一轮收口。独立仓库[TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy)的`v0.5.0`继续独立拥有快照、Revision/CAS、幂等、单记录与多记录原子事务、可查询事务回执、PostgreSQL权威存储、Redis已提交缓存与AOF backlog，以及多Endpoint和两个共享存储的无状态对等实例；主工程不得复制协议、连接Redis/PostgreSQL或导入`dbproxy-storage`。玩家聚合捕获已经投影为`inventory/progression/quest/runtime/wallet`五条记录和五个独立Revision：inventory保存Item，wallet保存金币，progression保存Numeric，quest保存任务，runtime保存地图状态、Buff和Skill/CD。商店提交inventory+wallet，玩家交易一次提交双方inventory+wallet；任务奖励和拾取提交inventory+quest；UseItem因Inventory、HP/MP、Buff与CD同时变化而提交inventory+progression+runtime。Map每秒错峰扫描、每次最多启动两个玩家保存且每图最多四个在途任务，默认30秒周期快照；下线与停机使用幂等最终Flush，停机每图最多八个并发worker并收集全部失败。`npm run test:player-trade:persistent`验证双Endpoint交易只提交一次；`npm run test:player-domain-recovery`验证最终Flush、all-in-one强杀恢复，以及精确强杀承载地图100的MapHost后Watcher收束和整组安全重启恢复。普通运行态最多允许回退一个快照周期，关键事务必须先可靠提交。Redis/PostgreSQL高可用由云厂商托管；透明MapHost热接管、批量Load/Save、Prometheus、TLS和生产部署仍未完成。完整步骤见[DBProxy玩家快照持久化](../tutorials/19-dbproxy-player-persistence.md)。
