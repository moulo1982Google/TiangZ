...
- 当前业务缺口按动态地图/Gate故障接管排序；无DBProxy时角色目录只保证进程生命周期内一致，重启恢复必须使用DBProxy。DBProxy v0.5.0的多Endpoint、双实例和TiangZ端到端故障切换已验收；当前开发分支进一步提供Prometheus/Grafana双实例观测，TiangZ Process会按有界Endpoint标签导出连接、请求失败和切换累计值。显式启用`lifecycle.restart`的静态MapHost已支持Watcher有界重启、Location代次接管和Gate在下一次进图时重新路由，但不恢复原Socket战斗现场或动态副本。
