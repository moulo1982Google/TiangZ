...
- Phase 4.5已完成玩家持久化第一轮收口。独立仓库[TiangZ-DBProxy](https://github.com/moulo1982Google/TiangZ-DBProxy)的`v0.5.0`继续独立拥有快照、Revision/CAS、幂等、单记录与多记录原子事务、可查询事务回执、PostgreSQL权威存储、Redis已提交缓存与AOF backlog，以及多Endpoint和两个共享存储的无状态对等实例；当前开发分支新增`LoadMultiSnapshot`，主工程用一次RPC恢复`inventory/progression/quest/runtime/wallet`五条记录和五个独立Revision。主工程不得复制协议、连接Redis/PostgreSQL或导入`dbproxy-storage`。Map默认30秒周期快照，下线与停机使用幂等最终Flush。`npm run test:player-domain-recovery`除最终Flush和all-in-one强杀恢复外，还验证显式配置的静态MapHost被精确强杀后Watcher保持存活并拉起新PID，Location以更高所有权代次清除旧Actor路由，Gate新连接再从DBProxy恢复五领域状态。`npm run test:player-trade:persistent`还验证首选Endpoint接管、Debug提交后响应丢失回执恢复，以及双Endpoint同时不可用时关键事务失败不改Entity、恢复后同一operationId只提交一次；统一入口为`npm run test:tiangz-fault-matrix`。普通运行态最多允许回退一个快照周期，关键事务必须先可靠提交。动态地图现场、Gate故障转移、批量Save、TLS和生产部署仍未完成。完整步骤见[DBProxy玩家快照持久化](../tutorials/19-dbproxy-player-persistence.md)。
