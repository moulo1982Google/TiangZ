...
- 2026-08-21的4000人、12 Gate复核暴露了Runtime Pump公平性边界：此前零错误单轮虽然完成Move/Probe/业务请求，但Map业务Update仅约11.1Hz、跨Grid仅达理论值41.1%；同参数复测进一步降到5.54Hz，Map正式窗口跳过395个固定帧，control/data ingress达到1025/3075生命周期峰值，并在2秒内明确拒绝425个Probe、106个CastSkill和90个UseItem。`SceneOverloaded`立即返回语义正确，但该点是不可接受且不稳定的拥塞悬崖。当前Pump在固定Update前最多处理512个Scene入站帧；后续优化必须以时间预算或控制/业务帧公平调度保证固定Tick，不能通过扩大队列、增加Gate或只看CPU掩盖失速。
