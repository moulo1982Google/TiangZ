...
受控 PG 连接阻塞已复现旧服务端在缓存成功后仍等待修复 DELETE 的问题。服务端现将成功提示交给原维护 worker：最多 1,024 个 key/revision，合并同 key 最高 revision，每轮最多 128 个再处理持久修复。没有增加连接或放宽超时；丢失、满载、取消及失败依赖原 PG 持久修复恢复，版本条件保护较新的目标。

该模式下 `committed_cache_sync` 只覆盖缓存尝试和本地提示登记，`cache_repair_ack` 是后台批量清理，不再是前者的子阶段。以下父子计时描述仅适用于旧版本和未启用延迟清理的独立 Store。此次精确回归及真实恢复测试不替代 100 玩家启动和 30 分钟故障验收，结果待本轮补充。
...
## 2026-09-07 提交后验收：启动健康检查失败

代码候选 DBProxy `e094541`；TiangZ 文档提交 `124dd7d` 后合并远端文档，实际冻结 HEAD 为 `34a4154`，构建输入未变。两仓库冻结时均干净。双仓库 release、正常 Model/Hotfix、游戏配置与两项业务探针重建通过；codegen 未留下生成物或协议锁差异。补齐新机器的 Rust 1.97.1、V8 跨盘构建目录联接和 .NET 8 运行时，保留初次环境失败日志。

两轮前置均通过 136 项默认测试、显式 31 项真实数据库/故障测试和 7 项控制器/资源测试。随后短时冒烟与正式计划都失败，不能把前置结果当作负载通过：

- `20260907-pgq-smoke-r1`：20 名游戏玩家首轮和交易通过；100 个持久化探针初始化时 PG 队列超过 2 秒，未发出 SOAK_READY，北京时间 17:57:40 失败。
- `20260907-pgq-30m-r1`：计划 1,800 秒、100 游戏玩家 + 100 持久化探针，18:05:35 启动，18:06:34 在首轮两图健康检查失败。共 34 次探针错误、57 次业务传输错误、38 次移动错误；没有 started.json/final.json，未进入正式计时、故障注入与最终对账。

正式启动现场两张地图 SDK 队列累计均值约 1.606/2.161 秒，共 46 个样本超过 5 秒，伴随 Actor/Scene RPC 超时；这不是稳态窗口。该轮没有相同 PG 排队超时日志，主 DBProxy 的启动采样 PG 队列 p99 在 500–1,000 ms 桶，修复 ACK p99 在 1–2 秒桶，不相加不同阶段 p99。本机 Docker 数据 VHD 位于机械盘，单次 8 KiB fdatasync 短时实测约 54 ms；这是明确的同步成本证据，未做 SSD 对照，不能声称所有游戏错误的唯一根因已经确认。

正式轮次最低可用内存 10.739 GiB、CPU 采样峰值 8.60%，资源守卫及指标抓取失败均 0。进程已清理、验收端口已释放、三只演练数据库容器健康。完整结果、构建日志位置与后续边界见[DBProxy 提交后验收记录](../../../TiangZ-DBProxy/docs/pg-queue-acceptance-2026-09-07.md)。未迁移 Docker 数据、未放宽超时或持久化配置，未执行完整 TiangZ quick/release/runtime 门禁、七天或 WoW335 实机验收。
