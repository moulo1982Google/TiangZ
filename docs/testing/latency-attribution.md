...
**最新结论：r6 在 NVMe PG 路径下完整 30 分钟、五次故障、最终对账及独立审计通过。** 冻结 DBProxy `0114af0` / TiangZ `d7af251`；北京时间 21:17:52–21:47:52 负载，21:48:20 最终对账通过。206,742 次成功读取，旧读/缺失/不变量错误均 0；AOF 64 条、819 条事件、1,638 条快照、819 条不可变事实和 2 条真实交易审计对账通过。保留 3 次恢复登录重试和所有暂时错误，详见[最终报告](../../../TiangZ-DBProxy/docs/storage-path-acceptance-2026-09-07.md)。机械盘 r4 仍为失败，NVMe 的效果不能算作代码收益；以下同日条目是当时过程记录。
